...
def CNN():
    ...
    model.add(
        tf.keras.layers.Conv2D(filters=10, kernel_size=2, activation="relu", padding="VALID", input_shape=(28, 28, 1)))

    model.add(tf.keras.layers.MaxPool2D(padding="VALID"))

    model.add(tf.keras.layers.Conv2D(filters=30, kernel_size=2, activation="relu", padding="VALID"))
    model.add(tf.keras.layers.MaxPool2D(padding="VALID"))

    model.add(tf.keras.layers.Conv2D(filters=30, kernel_size=5, activation="relu", padding="VALID"))
    model.add(tf.keras.layers.MaxPool2D(padding="VALID"))
